...
斐波拉契数列：数列从第3项开始,每一项都等于前两项之和，即： 1 + 2 + 3 + 5 + ... + (f(n-2) + f(n-1))
...
但是，上面的实现方式是低效的。以 `fibonacci(10)` 为例，它经过了以下计算：
...
上面的代码通过数组来做备忘录，但是遇到 n 特别大的情况呢？那时数组会占用很大的连续内存，则可能出现卡顿。如果处理这类场景呢？
...
案例：数独、八皇后、0-1 背包、图的着色、旅行商问题、全排列
...
> 树是一种非线性数据结构，常说的树都是查找树

树可以看成是链表的变体，链表中添加了left/right等属性从产生了树状结构

从节点之间位置关系的角度来看，二叉树的遍历分为 4 种。

1. 前序遍历：输出顺序根节点、左子树、右子树。
2. 中序遍历：输出顺序左子树、根节点、右子树。
3. 后序遍历：输出顺序左子树、右子树、根节点。
4. 层序遍历：广度遍历
